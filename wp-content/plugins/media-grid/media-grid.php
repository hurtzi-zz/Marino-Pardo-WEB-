...
function mg_global_scripts() {
	...
	wp_enqueue_style('mg-font-awesome', MG_URL . '/css/font-awesome/css/font-awesome.css');
	...
	if (is_admin()) {  
		...
		// iphone checks
		wp_enqueue_style( 'lcwp-ip-checks', MG_URL.'/js/iphone_checkbox/style.css', 999); // TO REMOVE IN NEXT RELEASE
	...
	if (!is_admin()) {
		...
		if(get_option('mg_js_head') != '1') {
			wp_enqueue_script('mg-frontend-js', MG_URL.'/js/frontend.js', 100, '3.123', true);	
		} else { 
			wp_enqueue_script('mg-frontend-js', MG_URL.'/js/frontend.js', 900, '3.123');
		}
...
$lcun = new lc_update_notifier(__FILE__, 'http://www.lcweb.it/envato_update/mg.php');
////////////
